class Query:
    ...
    @staticmethod
    def get_most_relevant_university_publications(
        university, keyword, start_year, end_year
    ):
        return """
        MATCH (i:INSTITUTE)<-[:AFFILIATION_WITH]-(f:FACULTY)-[:PUBLISH]->(p:PUBLICATION)-[l:LABEL_BY]->(k:KEYWORD)
        WHERE k.name = $keyword
    ...
    @staticmethod
    def get_most_relevant_faculty_publications(faculty, keyword, start_year, end_year):
        return """
        MATCH (i:INSTITUTE)<-[:AFFILIATION_WITH]-(f:FACULTY)-[:PUBLISH]->(p:PUBLICATION)-[l:LABEL_BY]->(k:KEYWORD)
        WHERE k.name = $keyword
            AND f.name = $faculty
    ...
    @staticmethod
    def get_publication_keywords(publication):
        return """
        MATCH (p:PUBLICATION)-[l:LABEL_BY]->(k:KEYWORD)
        WHERE p.title = $publication
        WITH k, l ORDER BY l.score DESC
